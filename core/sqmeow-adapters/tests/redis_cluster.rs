//! The Redis adapter against a Redis Cluster.
...
macro_rules! server {
    () => {
        match std::env::var("SQMEOW_TEST_REDIS_CLUSTER_URL") {
            Ok(url) => url,
            Err(_) => {
                eprintln!("skipped: set SQMEOW_TEST_REDIS_CLUSTER_URL, or run `just db-up`");
                return;
            }
        }
    };
...
async fn every_key_is_read_back_from_where_it_was_written() {
    ...
    for index in 0..30 {
        run(
            &backend,
            &format!("SET redis_cluster:spread:{index} v{index}"),
        )
        .await;
    }
    for index in 0..30 {
        let result = run(&backend, &format!("GET redis_cluster:spread:{index}")).await;
...
async fn the_drawer_lists_every_key() {
    let backend = Backend::connect(&server!()).await.unwrap();
    for index in 0..30 {
        run(&backend, &format!("SET redis_cluster:listed:{index} x")).await;
    }
    let relations = backend.relations("db0").await.unwrap();
    for index in 0..30 {
        let key = format!("redis_cluster:listed:{index}");
...
async fn a_hash_is_edited_in_place() {
    let backend = Backend::connect(&server!()).await.unwrap();
    run(&backend, "DEL redis_cluster:hash").await;
    run(&backend, "HSET redis_cluster:hash name al").await;

    let result = run(&backend, "HGETALL redis_cluster:hash").await;
    let changes = Changes {
